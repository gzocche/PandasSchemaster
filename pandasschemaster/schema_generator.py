"""
Schema Generator for PandasSchema library.
...
class SchemaGenerator:
    ...
    def _infer_object_type(self, series: pd.Series) -> np.dtype:
        ...
        try:
            ...
            if all(str(val).replace('.', '').replace('-', '').isdigit() for val in series.head(100)):
                return np.int64
            else:
                return np.float64
    ...
    def generate_schema_class_code(
        ...
        class_def.extend([
            f'    Generated with {len(schema_columns)} columns.',
            f'    """',
            ""
        ])
        ...
        for col_name, schema_col in schema_columns.items():
            ...
            class_def.append("    )")
            class_def.append("")        # Combine all parts
    ...
    def _clean_class_name(self, name: str) -> str:
        ...
        if not name.endswith('.csv') and not name.endswith('.xlsx') and not name.endswith('.json'):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate PandasSchema classes from data files"
    )
    parser.add_argument(
        "input_file",
        help="Path to the input data file (CSV, Excel, JSON, etc.)"
    )
    parser.add_argument(
        "-o", "--output",
        help="Output file path for the generated schema class"
    )
    parser.add_argument(
        "-c", "--class-name",
        help="Name for the generated schema class"
    )
    parser.add_argument(
        "-s", "--sample-size",
        type=int,
    ...
    parser.add_argument(
        "--no-nullable",
        action="store_true",
        help="Don't infer nullable columns"
    )
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
    ...
    try:
        ...
        generator = SchemaGenerator(
            infer_nullable=not args.no_nullable,
